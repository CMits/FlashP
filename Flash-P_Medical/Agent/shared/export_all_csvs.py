#!/usr/bin/env python3
"""
FLASH-M v2.0 (Medical edition) — Comprehensive CSV Export
...
NETWORKS list is populated as medical readout networks are created. Replace
this empty list (or extend it) with the directories you want included in the
cross-network merge. Each tuple is (relative_path, display_label).

If NETWORKS is empty, the script auto-discovers any `*_network/` subdirectory
of base_dir that contains both a `validation/` directory and a `network/`
directory, derives a display label from the directory name, and includes it.
"""
...
# Populate as medical networks come online. Example entries:
#   ("EGFR_NSCLC/cell_proliferation_network", "Cell Proliferation"),
#   ("p53_pathway/apoptosis_network", "Apoptosis"),
#   ("KRAS_PDAC/tumor_growth_network", "Tumor Growth"),
NETWORKS = []
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="FLASH-M v2.0 CSV Export")
    ...
    print(f"FLASH-M v2.0 CSV Export")
    print(f"Base: {base}")
