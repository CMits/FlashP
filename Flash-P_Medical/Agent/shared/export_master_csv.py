#!/usr/bin/env python3
"""
FLASH-M v2.0 (Medical edition) — Master CSV Export
...
NETWORKS list is populated as medical readout networks are created. If empty,
...
"Homo sapiens" for auto-discovered networks; override by listing explicitly.
"""
...
#   ("EGFR_NSCLC/cell_proliferation_network", "Cell Proliferation", "Homo sapiens (NSCLC)"),
#   ("p53_pathway/apoptosis_network", "Apoptosis", "Homo sapiens"),
NETWORKS = []
...
def _autodiscover_networks(base_dir: Path):
    ...
    for net_dir in sorted(base_dir.glob("**/*_network")):
        if net_dir.is_dir() and (net_dir / "validation").is_dir() and (net_dir / "network").is_dir():
            ...
            discovered.append((rel, label, "Homo sapiens"))
    return discovered
