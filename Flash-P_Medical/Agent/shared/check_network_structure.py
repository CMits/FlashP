...
NAMING_REGEX = {
    "GENE":             re.compile(r"^[A-Z][A-Z0-9_]*$"),       # EGFR, KRAS, TP53
    "PROTEIN_COMPLEX":  re.compile(r"^[A-Z][A-Z0-9_]*$"),       # PI3K, NF_KB, mTORC1
    "HORMONE":          re.compile(r"^[A-Z][A-Za-z0-9_-]*$"),   # EGF, TNF-Alpha, IL-6 (ligands/cytokines/growth factors)
    "METABOLITE":       re.compile(r"^[A-Z][A-Za-z0-9_]*$"),    # Lactate, Glucose, ROS
    "ENVIRONMENT":      re.compile(r"^[A-Z][A-Za-z0-9_]*$"),    # Hypoxia, Serum_Starvation, Radiation
    "PHENOTYPE":        re.compile(r"^[A-Z][A-Za-z0-9_]*$"),    # Cell_Proliferation, Apoptosis, Phospho_AKT
    "REGULATORY_RNA":   re.compile(r"^[a-z]+[0-9A-Z]"),         # miR21, miR34a, lncRNA42
    "DRUG":             re.compile(r"^[A-Z][A-Za-z0-9_-]*$"),   # Erlotinib, Osimertinib, Anti_PD1, Trastuzumab
}
