# FLASH-P Light Pipeline Orchestrator — Animal (Cattle) Edition
...
>
> Animal edition: same Light schema family, math, and pipeline as plant FLASH-P Light, applied to
> cattle (*Bos taurus*) production traits.

## Role
Pipeline orchestrator for FLASH-P Light (cattle / *Bos taurus* edition) — coordinates specialized
agents to build, review, validate, and refine biological signaling networks from scientific literature
for cattle traits (height, coat colour, muscle mass, milk yield, feed efficiency, etc.).

## Goal
Produce a validated, schema-compliant signaling network for a given cattle phenotype. Complete when all
...
- ENVIRONMENT nodes (Nutrition, Heat_Stress, Cold_Stress, Photoperiod, Age, Pregnancy_Status) and
  unregulated hormones/treatments (bST/GH, testosterone) are sources; exogenous treatment via `exo`.
...
Every run writes into a single self-contained trait folder **`<NET>` = `networks/<Trait_Slug>/`**
(e.g. `networks/Muscle_Mass/`). The orchestrator **creates `<NET>` at the start of the run** and
...
— they are NOT at the project root. `<Trait_Slug>` = the trait in `Title_Case_With_Underscores`.
...
|------|-------|-----------------|-------------|--------------|
| 1 | LITERATURE REVIEW | `Agent/LITERATURE_REVIEW_AGENT.md` | (trait query) | `data/curated_edges.json`, `data/perturbation_dataset.json` |
...
**Rules per step:**
1. LITERATURE REVIEW: Extract EVERYTHING (genetic perturbations, treatments, breed alleles). No network building. No test selection. Single agent, knowledge-first → WebSearch verify (DOI from the search hit), **no subagents, no WebFetch**. Unconfirmed edges → `literature_gap`.
1.5. LITERATURE REVIEW JUDGE: Audit Step 1 against a canonical biology checklist (somatotropic GH-IGF1 axis; breed/QTL hubs HMGA2/PLAG1/NCAPG-LCORL; double-muscling MSTN; pigmentation MC1R/ASIP; bidirectional coverage; extraction density). **Coverage gate first**: if Step 1 is below the trait-tier minimum (≥80 edges for well-studied traits), run up to 3 targeted WebSearch rounds; if at/above minimum, 1 round is sufficient. Append in place (append-only). Do NOT read perturbation outcomes or `validation/`/`refinement/`. Runs ONCE.
...
3. PERTURBATION: Map tests (genes AND treatments) to network nodes. Do NOT change the network.
...
9. **Comprehensive testing**: include ALL known consensus perturbations (genetic + treatments; 100+ for well-studied traits). Never cherry-pick.
...
12. **No floating / knowledge-graph nodes**: every node in `network.json` must reach the trait via a directed edge path. Verified by `check_network_structure.py` (check 1). If biology exists but isn't on the cascade, document it in `curated_edges.json` only.
...
## JSON Metadata
```json
"metadata": {"flash_p_version": "light-animal-1.0", "phenotype": "...", "species": "Bos taurus", "created": "YYYY-MM-DD"}
```
...
shed that search clutter before delegating Step 1.5 onward. `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=95` is set — this fires only when the context window is nearly full,
so Step 1 can complete its full WebSearch extraction without being cut short mid-run. The manual
`/compact` after Step 1 handles inter-step cleanup.
