# FLASH-P v2.0 Pipeline Orchestrator

**Author**: Christos Mitsanis | **Institution**: The University of Queensland

## Role

Pipeline orchestrator for FLASH-P v2.0 -- coordinates 8 specialized agents to build, review, validate, and refine biological signaling networks from scientific literature.
...
|------|-------|-----------------|-------------|--------------|
| 1 | LITERATURE REVIEW | `Agent/LITERATURE_REVIEW_AGENT.md` | (phenotype query) | `data/candidate_papers.json`, `data/curated_edges.json`, `data/perturbation_dataset.json` |
| 1.5 | LITERATURE REVIEW JUDGE | `Agent/LITERATURE_REVIEW_JUDGE_AGENT.md` | `data/candidate_papers.json`, `data/curated_edges.json`, `data/perturbation_dataset.json` | Updated in-place `data/candidate_papers.json`, `data/curated_edges.json`, `data/perturbation_dataset.json` (APPEND-ONLY merge, new IDs sequential), `data/literature_judge_report.json`, `data/_step1_snapshot/` (untouched Step 1 copies). |
...
| 2.5 | JUDGE | `Agent/JUDGE_AGENT.md` | `network/*.json`, `data/curated_edges.json`, `data/literature_judge_report.json` (NEVER perturbations/validation) | `network/judge_review_iteration_N.json`. Loops with BUILDER until verdict=`approved` or iteration=3. |
...
1.5. LITERATURE REVIEW JUDGE: Audit Step 1 output against a canonical biology checklist (pathways, hubs, canonical mutants, bidirectional coverage, extraction density). Run a targeted second-round WebSearch+WebFetch to close gaps. **APPEND-ONLY merge** — never delete, overwrite in-place, or renumber Step 1 entries. Snapshot Step 1 to `data/_step1_snapshot/` before any merge. New IDs continue sequentially from Step 1's last ID. Do NOT read perturbation-test outcomes or anything in `validation/` / `refinement/`. Runs ONCE per pipeline (not a loop).
2. BUILDER: Use ONLY edges from curated_edges.json (merged Step 1 + Step 1.5). Do NOT read perturbation results. Generate BOTH algebraic AND ODE equations. On iteration > 1, also read the JUDGE's review. Consult `literature_judge_report.json` `residual_gaps_for_builder` for unclosable gaps that should be flagged rather than invented around.
2.5. JUDGE: Biological quality review. Do NOT read perturbations or validation results. Suggests only; BUILDER applies. Max 3 iterations.
...
2.5 JUDGE --> reviews network (biology only, no tests) --> `judge_review_iteration_N.json` --> BUILDER applies --> repeat until `approved` or N=3
...
**CRITICAL: Use multiple parallel agents for LITERATURE REVIEW** -- launch 5-7 agents reading papers in parallel batches.
...
| Evidence | flat: `{doi, title, authors, ...}` | nested: `{claim, source: {doi, ...}}` |

---

## Non-Negotiable Rules

1. **Evidence standard**: Every edge needs a verified DOI with an evidence sentence. No fabricated references.
...
5. **Evidence carry-through**: DOI + title + evidence_sentence in ALL output files.
...
10. **curated_edges.json is the full repository**: ALL edges found in literature go into `curated_edges.json` with evidence. The BUILDER then selects which edges to use in the model.
11. **candidate_papers.json must include authors**: Every paper entry needs an `authors` field.
...
### Evidence format (FLAT -- v2.0)

Every evidence entry MUST use this flat structure:
```json
{
  "doi": "10.xxxx/...",
...
## Source-node percentage rule (softened from v2.0 spec)
...
{phenotype}_network/
  data/
    candidate_papers.json          <- ALL papers found (with authors, DOI, title, year, journal)
    curated_edges.json             <- ALL edges from literature (full repository, 150+)
    perturbation_dataset.json      <- ALL perturbation experiments found (100+)
    reconciled_perturbation_dataset.json  <- perturbations mapped to network nodes
  network/
...
| WebFetch returns paywall/403 | Use abstract from PubMed, log as "abstract_read" |
...
```json
"metadata": {"flash_p_version": "2.0", "phenotype": "...", "species": "...", "created": "YYYY-MM-DD"}
```
